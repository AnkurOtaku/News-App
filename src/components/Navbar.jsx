...
function Navbar() {
  ...
  return (
    <>
      <div className="h-18 py-1 px-2 md:py-4 min-w-full bg-[#A13737] grid grid-rows-2 grid-cols-3 justify-items-center md:grid-rows-1 items-center place-content-around gap-x-2">
        {/* heading */}
        <h1 className="px-4 text-2xl lg:text-3xl col-span-2 md:col-span-1 order-1 text-white font-semibold self-center">
          Worldwide News
        </h1>

        {/* search bar */}
        <form
          className="order-3 md:order-2 col-span-2 flex w-4/5 md:col-span-1"
          onSubmit={(e) => handleSubmit(e)}
        >
          <div className="rounded-md w-full flex justify-center items-center bg-[#A75F5F]">
            <input
        ...
        <div className="md:order-3 order-2 row-span-2 h-full flex items-center justify-around flex-col md:flex-row">
          {/* update icon */}
          <button
            ...
            {dropdown && (
              <div className="origin-top-right absolute z-[1] right-0 mt-2 rounded-md shadow-lg">
                <div className="rounded-md bg-white h-80 overflow-y-auto shadow-xs">
                  ...
                  >
                    <a
                      className={`block w-full px-4 py-2 rounded-md text-center text-sm text-gray-700 ${
                        !country
                          ? "bg-black text-white"
                    ...
                    {countryList.map((item) => (
                      <button
                        ...
                        className={`block w-full px-4 py-2 rounded-md text-sm text-gray-700 ${
                          country && item.country === country.country
                            ? "bg-black text-white"
                            : "hover:bg-gray-100 hover:text-gray-900"
                        }`}
                        role="menuitem"
